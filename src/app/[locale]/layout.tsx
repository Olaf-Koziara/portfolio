      ...
      <body className="antialiased">
        <ThemeProvider>
          <NextIntlClientProvider messages={messages}>
            <a
              href="#main-content"
              className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 z-[100] bg-accent text-white px-4 py-2 rounded-lg shadow-lg"
            >
              {t("skipToContent")}
            </a>
            {/* Navigation */}
            ...
            <footer className="border-t border-border py-8 px-4 sm:px-6 lg:px-8">
              <div className="max-w-6xl mx-auto text-center text-muted text-sm">
                <p>
                  © {new Date().getFullYear()} Olaf Koziara {tFooter("rights")}
                </p>
              </div>
            </footer>
